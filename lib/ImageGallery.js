...
class ImageGallery extends React.Component {
  ...
  _animateZoomIn = (props = this.props) => {
    ...
    if (USE_NATIVE_DRIVER) {
      innerContentValue.__makeNative();
      zoomInOutValue.__makeNative();
    }
  ...
  _animateAutomatedSlideOut = () => {
    Animated.timing(this.state.innerContentValue, {toValue: 0, duration: 150, useNativeDriver: USE_NATIVE_DRIVER }).start();
  ...
  _onPanResponderMove = (e, {dy}) => {
    if (this._responderGranted) {
      Animated.timing(this.state.innerContentValue, {toValue: 0, duration: 250, useNativeDriver: USE_NATIVE_DRIVER }).start();
  ...
  _onPanResponderEnd = (e, {moveX, moveY, dx, dy, vx, vy}) => {
    ...
    if (Math.abs(dy) >= throwThreshold) {
      this._animateSlideOut(vy, dy);
    } else {
      Animated.spring(panValue, {toValue: 0, speed: 80, bounciness: 8, useNativeDriver: USE_NATIVE_DRIVER }).start();
      Animated.parallel([
...
reactMixin(ImageGallery.prototype, TimerMixin);
export default connect(
  data => ImageGallery.getDataProps(data),
)(ImageGallery);
